use crate::lex::constants::{LexItem::{*, self}, NumberType};
...
macro_rules! grammar {
    ...
    ($($lefttoken:ident -> $($righttoken:expr)*),* $(,)*) => {
        &[
            $(
                ($lefttoken, grammar!(@line $($righttoken),*))
            ),*
        ];
    };
}

macro_rules! grammar2 {
    (@token StringLiteral) => {
    ...
    (@beginline $token:ident -> $($tail:tt)*) => {
        grammar2!(@replacement ($token, vec![]) $($tail)*)
    };
    (@replacement $parens:tt , $($tail:tt)*) => {
        grammar2!(@beginline $($tail)* $parens)
    };
    (@replacement $parens:tt ε $($tail:tt)*) => {
        grammar2!(@replacement $parens $($tail)*)
    };
    (@replacement ($name:tt, vec![$($expression:tt)*]) $token:tt $($tail:tt)*) => {
        grammar2!(@replacement ($name, vec![$($expression)* grammar2!(@token $token), ]) $($tail)*)
    };
    (@eof $($tail:tt)*) => {
        vec![$($tail),*]
    };
    (run $($tail:tt)*) => {
        grammar2!(@beginline $($tail)* @eof)
    };
}
